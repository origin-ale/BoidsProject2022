...
Ring::Ring(const float radius, const float hole,
...
float Ring::getRadius() const { return m_radius; }
...
float Ring::getHole() const { return m_hole; }
...
unsigned int Ring::getNumberOfSides() const { return m_numberOfSides; }
...
sf::Color Ring::getColor() const { return m_color; }
...
void Ring::setTexture() { m_texture = nullptr; }
...
sf::FloatRect Ring::getLocalBounds() const {
  if (m_vertices.size() == 0) return {{0.f, 0.f}, {0.f, 0.f}};
...
float Ring::getSectorSize() const { return m_sectorSize; }
...
float Ring::getSectorOffset() const { return m_sectorOffset; }
...
void Ring::draw(sf::RenderTarget& target, sf::RenderStates states) const {
  ...
  if (m_vertices.size() > 0)
    target.draw(&m_vertices.front(), m_vertices.size(), m_primitiveType,
                states);
}
...
void Ring::priv_updateVertices() {
  ...
  for (std::vector<sf::Vertex>::iterator begin{m_vertices.begin()},
       end{m_vertices.end()}, it{begin};
       it != end; ++it) {
    ...
    it->position = {
        m_radius + std::sin(angle) * m_radius * (isInnerPoint ? m_hole : 1.f),
        m_radius - std::cos(angle) * m_radius * (isInnerPoint ? m_hole : 1.f),
    };
    it->color = m_color;
    const sf::Vector2f scaledPosition{it->position / (m_radius * 2.f)};
    it->texCoords = {
        scaledPosition.x * m_textureRect.width + m_textureRect.left,
        scaledPosition.y * m_textureRect.height + m_textureRect.top};
    ;
  }
}
