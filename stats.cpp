...
std::vector<double> getFlockDistances(std::vector<Boid> const& boids,
                                      int flock) {
  std::vector<double> dists;
  for (int j = 0; j < static_cast<int>(boids.size()); ++j) {
    for (int k = j + 1; k < static_cast<int>(boids.size()); ++k) {
      if (boids[j].getFlock() == flock &&
          boids[j].getFlock() == boids[k].getFlock())
        dists.push_back(
            sqrt((boids[j].getPosition() - boids[k].getPosition()).getNorm2()));
    }
  }
  return dists;
}

std::vector<double> getPredFlockDistances(std::vector<Boid> const& boids,
                                          int flock,
                                          std::vector<Boid> const& preds) {
  std::vector<double> dists;
  for (int j = 0; j < static_cast<int>(preds.size()); ++j) {
    for (int k = 0; k < static_cast<int>(boids.size()); ++k) {
      if (boids[k].getFlock() == flock)
        dists.push_back(
            sqrt((preds[j].getPosition() - boids[k].getPosition()).getNorm2()));
    }
  }
  return dists;
}
...
void fillDrawHisto(TH1D& histo, std::vector<double> const& data,
                   Color_t const& histo_color, float statbox_offset) {
  ...
  TPaveStats statbox = TPaveStats(x_center - x_width, y_center + y_width,
                                  x_center + y_width, y_center - y_width, "NB");
  ...
  std::string stat_name =
      stat_name_histo + stat_name_base + std::to_string(statbox_offset);
  ...
  TText* histo_mean_txt =
      new TLatex(0, 0, (histo_mean_label + histo_mean).c_str());
  ...
  TText* histo_stdev_txt =
      new TLatex(0, 0, (histo_stdev_label + histo_stdev).c_str());
