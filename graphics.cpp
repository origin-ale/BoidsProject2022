...
void initializeGraphic( Boid const& boid, 
...
void updateGraphic(Boid const& boid, sf::CircleShape& graphic, sw::Ring& sight_arc, sf::RenderWindow& target_window, double scale){
  ...
  sight_arc.setPosition(scale * boid.getPosition().getX(), - scale * boid.getPosition().getY());
